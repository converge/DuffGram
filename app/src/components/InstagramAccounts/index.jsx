...
class AddIgAccount extends React.Component {
	...
	if (!this.props.tmpUsername.length) {
	    action_button = <Button onClick={this.props.triggerCreateIgAccount} color="primary">Add</Button>
	} else {
	...
	return (
		...
		<Dialog
			...
			<TextField
			    ...
			    value={ (this.props.tmpUsername) ? this.props.tmpUsername : this.props.currentUsername} />
			<TextField
			    id="password"
			    name="ig_password"
			    onChange={event => this.props.handleChangeValue(event)}
...
export default class InstagramAccounts extends React.Component {
	...
	let rows = this.state.data.map(username => {
	    return <Usernames key={username.id}
		data={username}
...
const Usernames = props => {
	...
	<TableRow>
	    ...
	    <TableCell>
		<Button color="primary" onClick={() => props.onUpdate(props.data.id)}>Change Password</Button>
		<Button color="secondary" onClick={() => props.onDelete(props.data.id)}>Delete</Button></TableCell>
	</TableRow>
    );
};
